...
class UpdateUserDTO extends BaseDTO
{
    ...
    public static function validate(array $data): array
    {
        ...
        if (array_key_exists('phone_number', $data)) {
            if ($data['phone_number'] !== null && $data['phone_number'] !== '' && !preg_match('/^[\+]?[1-9][\d]{0,15}$/', $data['phone_number'])) {
                throw new \InvalidArgumentException('Phone number format is invalid');
            }
            $validated['phone_number'] = $data['phone_number'] !== null && $data['phone_number'] !== '' ? $data['phone_number'] : null;
        }
